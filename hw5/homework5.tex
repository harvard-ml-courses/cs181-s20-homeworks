...
\duedate{11:59pm April 10, 2020} % Adjust date
...
% FDV: Changed to Beta-Geometric just to change things up, will need
% to be re-written for that.  I think the math should all work out but
% let me know if there is trouble (Geometric -> Negative Binomial,
% should then be cleanly conjugate...) Also, thinking to add a coding
% component -> would it make it too long?
...
\item Suppose that this had been a classification problem. That is,
  ...
  % FDV: Added this: let's see how hard it is and tweak the
  % probabilities to get different effects (might be too tricky with
  % geometrics, in which case we can make the question more about why
  % it is tricky -> e.g. would adding a dimension help)
...
% FDV: This problem is the same as last year.
...
\newpage
  ...
  % FDV: From previous T5, Minor notation changes + changed the
  % network.  Adjust the solutions for the new network with the new
  % names.  Please check carefully.
